...
public static class ProxyConfigurationProjectionMapper
{
        ...
        return new ProxyConfigurationProjection(
            ...
            new RuntimeAcmeProjection(
                ...
                snapshot.Acme.Certificates
                    .Select(ToAcmeCertificateProjection)
                    .ToArray()),
            ...
            snapshot.Certificates.Values
                .Select(ToCertificateProjection)
            ...
            new ReadOnlyCollection<RuntimeListenerProjection>(snapshot.Listeners
                .Select(ToListenerProjection)
                .ToArray()),
            new ReadOnlyCollection<RuntimeRouteProjection>(snapshot.Routes
                .Select(ToRouteProjection)
                .ToArray()));
    }

    private static RuntimeAcmeCertificateProjection ToAcmeCertificateProjection(
        RuntimeAcmeCertificateOptions certificate)
    {
        ArgumentNullException.ThrowIfNull(certificate);

        return new RuntimeAcmeCertificateProjection(
            certificate.Id,
            certificate.Enabled,
            certificate.Domains,
    ...
    private static RuntimeCertificateProjection ToCertificateProjection(RuntimeCertificate certificate)
    {
        ArgumentNullException.ThrowIfNull(certificate);

        return new RuntimeCertificateProjection(
            certificate.Id,
            certificate.Path,
            certificate.Format,
            certificate.Source,
    ...
    private static RuntimeListenerProjection ToListenerProjection(RuntimeListener listener)
    {
        ArgumentNullException.ThrowIfNull(listener);

        return new RuntimeListenerProjection(
            listener.Name,
            listener.Address,
            listener.Port,
            listener.Enabled,
            ...
            listener.SniCertificates.Select(ToSniCertificateBindingProjection).ToArray(),
            listener.Backlog,
    ...
    private static RuntimeSniCertificateBindingProjection ToSniCertificateBindingProjection(
        RuntimeSniCertificateBinding binding)
    {
        ArgumentNullException.ThrowIfNull(binding);

        return new RuntimeSniCertificateBindingProjection(
            binding.HostName,
            binding.CertificateId);
    }

    private static RuntimeRouteProjection ToRouteProjection(RuntimeRoute route)
    {
        ArgumentNullException.ThrowIfNull(route);

        return new RuntimeRouteProjection(
            route.Name,
            route.Host,
            route.PathPrefix,
            route.Action,
            ...
            new RuntimeHeaderPolicyProjection(
                route.HeaderPolicy.SetRequestHeaders.Select(ToHeaderFieldProjection).ToArray(),
                route.HeaderPolicy.RemoveRequestHeaders,
                route.HeaderPolicy.SetResponseHeaders.Select(ToHeaderFieldProjection).ToArray(),
    ...
    private static RuntimeUpstreamProjection ToUpstreamProjection(RuntimeUpstream upstream)
    {
        ...
        return new RuntimeUpstreamProjection(
            upstream.RouteName,
            upstream.Name,
            upstream.Scheme,
            upstream.Protocol,
    ...
    private static RuntimeHeaderFieldProjection ToHeaderFieldProjection(ProxyHeaderField header)
    {
        ...
        return new RuntimeHeaderFieldProjection(header.Name, header.Value);
    }
